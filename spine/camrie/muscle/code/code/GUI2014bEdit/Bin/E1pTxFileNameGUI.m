function [E1pFileName,E1pFileAddr,index] = E1pTxFileNameGUI(Tx,filePath,fileName,Mag,Phs,FA)
    ...
    for n=1:Tx
        ...
        if ~strcmp(fileName,'')
            ...
            eval(['set(',temphFileNameEdit,',''UserData'','''',''String'',''E1+ Filename'',''FontAngle'',''italic'');']);
        end;
    ...
    uiwait(gcf);
    if tempBool == 0
        E1pFileName = fileName; E1pFileAddr = filePath; index = 0;
    else
        for n=1:Tx
           temphFileNameEdit = ['hTxFileNameEdit',num2str(n)];
           tempStr = eval(['get(hTxFileNameEdit',num2str(n),',''UserData'');']);
           [tempPath,tempName] = addrCvrt(tempStr);
           eval(['E1pFileAddr{',num2str(n),'} = tempPath;']);
           eval(['E1pFileName{',num2str(n),'} = tempName;']);
        end
        index = 1;
        clear tempStr;
    end
...
function TxFileName_Callback(source,eventdata,hTxFileNameEdit,Tx)
    ...
    [fileName,pathName,index]=uigetfile({'*.efld','E1 Plus File (*.efld)';'*.*','All Fiiles (*.*)'},'OPEN: E1 Plus File');
    if index == 1
