...
bool TissIn::readTissData()
{
	...
	while (!fHandle.eof())
	{
		...
		// T1, from ms.
		tempTissType.t1 = (float)strtod(strT1.c_str(), NULL)/1000; // scale into seconds from ms in file
		
		// T2, from ms
		tempTissType.t2 = (float)strtod(strT2.c_str(), NULL)/1000; // scale into seconds from ms in file
		
		// Rho
		tempTissType.rho = (float)strtod(strRho.c_str(), NULL);

		// SigmaChem
		tempTissType.sigmaChem = (float)strtod(strSigmaChem.c_str(), NULL);
