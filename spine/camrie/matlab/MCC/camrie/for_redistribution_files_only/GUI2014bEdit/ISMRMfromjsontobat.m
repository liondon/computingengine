...
function batfile = ISMRMfromjsontobat(jsonfile, OutputJson,logJson,parentdir,SUBSAMPLES)
MAINPATH=pwd;
batfile =0;
...
try
    inputInfo = readInput(filetest, 1);
...
%#####DEBUG####
slicemin=min(SUBSAMPLES(:));
slicemax = max(SUBSAMPLES(:));
...
for coil = 1:(val.coils.receivingCoilsNumber)
    ...
    OutClass.addToExporter('image2D', namevarSig, KSigMat(:,:,SUBSAMPLES,coil));
end
%temp = strcat(val.path, OutputJson);
OutClass.exportLOG(logJson);     OutClass.exportResults(OutputJson)
...
try
    ...
    for coil = 1:(val.coils.receivingCoilsNumber)
        ...
        OutClass.addToExporter('image2D', namevarSigIm, ImDataCoils(:,:,SUBSAMPLES,coil));
...
try
    ...
    OutClass.addToExporter('image2D', 'K Matrix Data', KDataMat(:,:,SUBSAMPLES));
    OutClass.addToExporter('image2D', 'Image Matrix Data', ImData(:,:,SUBSAMPLES));
    %temp = strcat(val.path, OutputJson);
     %    OutClass.exportLOG(logJson);     OutClass.exportResults(OutputJson)
catch
