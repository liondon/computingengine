...
function seqfile = sequencewrite(val, inputInfoOld)
...
%         cTxNum = 1;
        ...
        if index ~= 0
            Tx.MagMat = ones(val.coils.transmittingCoilsNumber,1);
            Tx.PhsMat = zeros(val.coils.transmittingCoilsNumber,1);
            Tx.Num = val.coils.transmittingCoilsNumber;
            
            for coils = 1:(val.coils.transmittingCoilsNumber)
                if isempty(val.fields.b1plus)<1
                    Tx.MagMat(coils) = val.fields.b1plus(coils).amplitude;
                    Tx.PhsMat(coils) = val.fields.b1plus(coils).phase;
                end
            end
%             
%             Tx.MagMat = cMagMat;
...
%             Tx.Num = cTxNum;
            ...
            Rx.Num = val.coils.receivingCoilsNumber;
...
%             TI.duration = str2double(get(hTI,'string')); %ms
            TI.duration = val.sequence.TI;
%             MatSize.value = get(hMatSize,'value');
            MatSize.value(1) = val.sequence.matrixsize0;
            MatSize.value(2) = val.sequence.matrixsize1;
%             FOV.value = get(hFOV,'value'); %mm
            FOV.value(1) = val.sequence.fov0;
            FOV.value(2) = val.sequence.fov1;
%             RF.Duration = str2double(get(hRF_Duration,'String')); %ms
            RF.Duration = val.sequence.rfpd;
%             RF.Num = str2double(get(hRF_Num,'String'));
            RF.Num=val.sequence.nrfp;
%             RF.Shape = get(hRF_Shape,'Value');
            RF.Shape = val.sequence.rfshape;
            
%             ADC.BW = str2double(get(hADC_Freq,'String')); %kHz
            ADC.BW = val.sequence.rbw;

%             GradOn.ss = get(hSS,'Value'); %gradients are specified in mT/m, but here it is a on/off flag.
            GradOn.ss = val.sequence.ss;
%             GradOn.fe = get(hFE,'Value');
            GradOn.fe = val.sequence.fe;
%             GradOn.pe = get(hPE,'Value');
            GradOn.pe = val.sequence.pe;
            %GradOn.pe=1;
...
%             temp = get(hPEDirectMenu,'String'); peDirectoin = temp(get(hPEDirectMenu,'Value')); clear temp;
            ...
            peDirectoin = peDirect(val.sequence.pedirection);

%             dummy = str2double(get(hDummyNum,'String'));
            dummy = val.sequence.dTR;

%             slice.TH = str2double(get(hGeoSliceTH,'String'))/1000;
            slice.TH = val.sequence.slicethickness/1000;
            
            valuesw = val.sequence.sliceorientation;
...
%             tempNum = get(hSeqType,'Value');
            %tempNum = cSeqType;
            tempNum = val.sequence.sequencename;
%             load('SeqTypes.mat');
...
%             finalSeqData = combineSeq(interSeqData,get(hOrientationMenu,'Value'),peDirectoin,dummy); %
            ...
            seqfile = expSeq(finalSeqData,dummy,Tx,Rx, val); %
...
%             end;
    ...
    function seqfile = expSeq(finalSeqData,dummy,Tx,Rx, val)
...
%         tempStr = get(hStudyName,'String');
        ...
        seqfile = strcat(val.path, tempStr, '.seqn');
