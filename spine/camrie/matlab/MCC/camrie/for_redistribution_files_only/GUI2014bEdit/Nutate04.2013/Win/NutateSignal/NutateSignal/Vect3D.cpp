...
void rotate(Vect3D<T> & vect, Vect3D<T> & axis, T theta)
{
	if (!(axis == 0))
	{
		...
		// these calculations cause some errors -> Turned out no errors, but slower for speed.
		...
		//{
			...
			// http://inside.mines.edu/~gmurray/ArbitraryAxisRotation/ArbitraryAxisRotation.html (Eqn 5.1)
