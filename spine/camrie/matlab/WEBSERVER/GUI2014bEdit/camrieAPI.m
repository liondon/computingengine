...
function OUTPUT = camrieAPI(jsonfile, OutputJson,logJson,parentdir)
OUTPUT =[];


functionspath=pwd;
%if it's a link download it
[a,b,c]=fileparts(jsonfile);
...
try
    ...
    KSigMat = zeros(val.sequence.matrixsize0, val.sequence.matrixsize1, slicemax-slicemin+1, val.coils.receivingCoilsNumber);
    KDataMatCoils = zeros(val.sequence.matrixsize0, val.sequence.matrixsize1, slicemax-slicemin+1, val.coils.receivingCoilsNumber);
    KDataMat = zeros(val.sequence.matrixsize0, val.sequence.matrixsize1, slicemax-slicemin+1);
    ...
    for slice = slicemin:slicemax
        ...
        try
            for coil = 1:(val.coils.receivingCoilsNumber)
                ...
                KSigMat(:,:,slice-slicemin+1,coil) = loadSignal(sigfile, val, x, y);
                KDataMatCoils(:,:,slice-slicemin+1, coil) = KSigMat(:,:,slice-slicemin+1,coil)+KNoise(:,:,coil);
...
ImDataCoils = zeros(siImage(1), siImage(2), slicemax-slicemin+1, val.coils.receivingCoilsNumber);
ImData = zeros(siImage(1), siImage(2), siKdata(3));

try
    for slice = slicemin:slicemax
        for coil = 1:val.coils.receivingCoilsNumber
            ImDataCoils(:,:,slice-slicemin+1, coil) = fromktoimageSingle(KDataMatCoils(:,:,slice-slicemin+1, coil), peKey);
        end
    end
    ...
    for slice = slicemin:slicemax
        for recCoil = 1:nRx
            if sumMethod==1 %Sum of Square
                ImData(:,:,slice-slicemin+1)=ImData(:,:,slice-slicemin+1)+abs(ImDataCoils(:,:,slice-slicemin+1,recCoil)).^2;
                KDataMat(:,:,slice-slicemin+1)=KDataMat(:,:,slice-slicemin+1)+abs(KDataMatCoils(:,:,slice-slicemin+1,recCoil)).^2;
            else %Sum of Magnitude
                ImData(:,:,slice-slicemin+1)=ImData(:,:,slice-slicemin+1)+abs(ImDataCoils(:,:,slice-slicemin+1,recCoil));
                KDataMat(:,:,slice-slicemin+1)=KDataMat(:,:,slice-slicemin+1)+abs(KDataMatCoils(:,:,slice-slicemin+1,recCoil));
            end
        end
    end
    if sumMethod==1
