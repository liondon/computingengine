...
VERTEBRANAME=os.getenv('NAME',"Block Equation Solver (PSUdoMRI)")
VERTEBRAID=os.getenv('ID','2')
...
#     return "sh " + os.path.join(MATLABCODEDIR,PROGRAM)
...
def efference():
    if request.is_json:       
        ...
        if isUser(user,pwd):
            ...
            task = celery.send_task('camrie.cu', args=[joptions], kwargs={})
            # result.wait()
