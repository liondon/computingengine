function dgf_sphere_plot_coilSAR_parTx(sar,g,localsar,profile,globalsar,peaksar,...
    ...
    plotsar,localsar_movie,plotcurrents,currents_plot_3D,currents_movie,...
...
%     plotprofile,movie_compression,movie_fps,commondir,plotdir,moviedir,user_label)
% 
% Plot routines for circular coils SAR, circular coils transmit g-factor, optimal weights
...
plotlabel = [num2str(ncoils) ' Circular Coils, B_0=' num2str(fieldstrength) 'T, FOV=' num2str(100*fovf) 'cm x ' num2str(100*fovp) 'cm, R=' ...
...
if plotsar,
    if isnan(sarmean)
        disp('*************************************')
        disp('coil SAR not available for plotting')
        disp('*************************************')
    else
        figure
        ...
        imshow(real(sar)',[]);
        colorbar
        title(['mean coil SAR = ' sprintf('%0.2f',sarmean) ', max coil SAR = ' sprintf('%0.2f',sarmax)  datelabel])
        colormap('jet');
        ...
        imshow(real(g)',[]);
        colorbar
        title(['mean coil g = ' sprintf('%0.2f',gmean) ', max coil g = ' sprintf('%0.2f',gmax)  datelabel])
        colormap('jet');
    end
end
...
if plotprofile,
    figure;
    set(gcf,'name',['Actual Excited Profile in 3D - parallel Tx - ' num2str(ncoils) ' circular coils']);
    surf(x_fov,y_fov,real(profile));
    title(['Actual Excited Profile'])
    iptsetpref('ImshowInitialMagnification','fit');
    figure;
    set(gcf,'name',['Actual Excited Profile - parallel Tx - ' num2str(ncoils) ' circular coils']);
    imshow(real(profile),[]);
    title(['Actual Excited Profile'])
end
...
if plotcurrents | currents_plot_3D,
    for ipatterns = 1:size(whichvoxelpatterns,1),
        ...
        plotlabel_current = char(...
            ['Coil current pattern for parallel Tx (b=' num2str(100*outer_radius,2) 'cm, B_0=' num2str(fieldstrength,2) ...
        ...
        set(gcf,'name',['Coil current pattern for parallel Tx ' num2str(fieldstrength,2) 'T, b=' num2str(100*outer_radius,2) 'cm, (rho,phi,theta)=' num2str(rhovox*100,3) 'cm,' num2str(phivox,3) 'deg,' num2str(thetavox,3) 'deg), ' num2str(acceleration_factor(1)) 'x' num2str(acceleration_factor(2)) ' accel'])
        drawnow
    end
end
...
%         end
    ...
    mov = immovie(X,Map);
    moviefilename = [moviedir '/Coil_current_movie_' num2str(ncoils) 'ele_matrix' num2str(nf) 'x' num2str(np) '_' num2str(fieldstrength) 'T_acc' num2str(acceleration_factor(1)) 'x' num2str(acceleration_factor(2)) '.avi'];
...
%         end
    ...
    mov = immovie(X,Map);
    moviefilename = [moviedir '/CoilSAR_movie_' num2str(ncoils) 'ele_matrix_' num2str(nf) 'x' num2str(np) '_' num2str(fieldstrength) 'T_acc' num2str(acceleration_factor(1)) 'x' num2str(acceleration_factor(2)) '.avi'];
