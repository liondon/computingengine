...
% -------------------------------------------------------------------------
%   DGF_sphere_snr_batch.m
%
%   Set up ultimate SNR calculations for a spherical phantom,
%   including ideal current patterns and SNR of finite arrays
%
%   References:
%
...
%
% NOTES:
%       added Poynting vector calculation (RL and MV June 7, 2012)
%       major upgrade to include EM field and conductive losses in multiple (up to 4) layers (RL October 7, 2014)
%
% -------------------------------------------------------------------------
...
fieldstrength_set = [1.5]; %[1 3 5 7 9 11];
...
snr_radius = 1; % 1 = radius1 (outermost layer), 2 = radius2, 3 = radius3 (innermost layer). This specify the FOV of the SNR map
if (snr_radius ~= 1) && (snr_radius ~= 2) && (snr_radius ~= 3)
    disp('-------------------------------------------------------');
    disp('**ERROR** snr_radius can only be equal to 1, 2, or 3   ');
    disp('-------------------------------------------------------');
    keyboard
end
switch snr_radius
...
radius3_set = [0.08];  %[0.0870]% [0.0843]; %[0.05 0.15 0.25];  % [m] (innermost radius)
nradin = length(radius3_set);
tissue_r4 = [];
% use tissue_r4 = [] to manually set tissue properties below
...
radius2_set = radius3_set + 0.01; % radius3 < radius < radius2
tissue_r3 = [];
% use tissue_r3 = [] to manually set tissue properties below
...
radius1_set = radius3_set + 0.02; % radius3 < radius < radius2
tissue_r2 = [];
...
currentradius_set = [0.12];     % [m] radius of the surface where the coil/ultimate current density is defined (outermost radius)
...
% **--**       **--**     **--**       **--**     **--**       **--**     **--**
outerfov_radius_set = currentradius_set + 0.01; % how large the FOV for the fields calculations (for radiated fields)
...
if size(current_set,2) < size(currentradius_set,2)
    disp('-------------------------------------------------------');
    disp('**ERROR** size of current_set smaller than size of currentradius_set');
    disp('-------------------------------------------------------');
    keyboard
end
if size(current_set,2) > size(currentradius_set,2)
    disp('------------------------------------------------------------------------------------------------------------------------------');
    disp('**WARNING** current_set larger than currentradius_set, only current choices between 1 and size(currentradius_set,2) will be used');
    disp('------------------------------------------------------------------------------------------------------------------------------');
    keyboard
end
...
compute_ultSNR_flag = 1;                 % 1 --> compute ultimate intrinsic SNR
compute_coilSNR_flag = 1;                % 1 --> compute SNR for circular surface coils
compute_ult_current_pattern_flag = 1;    % 1 --> compute ultimate current patterns
compute_coil_current_pattern_flag = 0;   % 1 --> compute coil current patterns
compute_ult_field_patterns = 0;          % 1 --> compute ultimate E and B field patterns
compute_coil_field_patterns = 0;         % 1 --> compute coil E and B field patterns
compute_patterns_with_stored_weights = 0;% 1 --> use weights stored in users's choice of file ('weights_file') to compute current and field patterns, in place of computed weights 
...
%--- saved variables ---%
save_emfields_ult_r4_flag = 0;           % 1 --> save a matrix with the modes' electric and magnetic fields in Region 4 (WARNING: large file)
save_emfields_coil_r4_flag = 0;          % 1 --> save a matrix with the coils' electric and magnetic fields in Region 4
save_emfields_ult_r3_flag = 0;           % 1 --> save a matrix with the modes' electric and magnetic fields in Region 3 (WARNING: large file)
save_emfields_coil_r3_flag = 0;          % 1 --> save a matrix with the coils' electric and magnetic fields in Region 3
save_emfields_ult_r2_flag = 0;           % 1 --> save a matrix with the modes' electric and magnetic fields in Region 2 (WARNING: large file)
save_emfields_coil_r2_flag = 0;          % 1 --> save a matrix with the coils' electric and magnetic fields in Region 2
save_emfields_ult_r1_flag = 0;           % 1 --> save a matrix with the modes' electric and magnetic fields in Region 1 (WARNING: large file)
save_emfields_coil_r1_flag = 0;          % 1 --> save a matrix with the coils' electric and magnetic fields in Region 1
save_ult_current_weights_flag = 0;       % 1 --> save ultimate current weights
save_coil_current_weights_flag = 0;      % 1 --> save circular surface coil current weights
save_ult_currents_flag = 0;              % 1 --> save ultimate current patterns
save_coil_currents_flag = 0;             % 1 --> save coil current patterns
save_noise_contributions = 0;            % 1 --> save sample, coil and radiation losses separately
...
plot_ultSNR = 1;                        % 1 --> plot ultimate SNR results
plot_coilSNR = 1;                       % 1 --> plot SNR results for circular surface coils
plot_ult_current_patterns = 1;          % 1 --> plot optimal current patterns
plot_ult_current_weights = 0;           % 1 --> plot modes' combination weights for ultimate SNR
plot_coil_current_patterns = 0;         % 1 --> plot surface coil current patterns
plot_coil_current_weights = 0;          % 1 --> plot coils' combination weights for optimal SNR
...
plot_efields_ult = 0;                   % 1 --> plot the net modes' electric field for regions where it was saved. It uses SoS combination or weighted combination if weights are saved.
plot_efields_coil = 0;                  % 1 --> plot the electric fields of the individual coils for regions where they were saved (SoS or weighted combination if weights are saved)
plot_bfields_ult = 0;                   % 1 --> plot the net modes' magnetic field for regions where it was saved. It uses SoS combination or weighted combination if weights are saved.
plot_bfields_coil = 0;                  % 1 --> plot the magnetic fields of the individual coils for regions where they were saved (SoS or weighted combination if weights are saved)
...
plot_regions_fov = 1;                   % 1 --> plot the 2D FOVs associated with each region (possible only if preset_fov_geometry_flag = 0)
...
include_ult_conductor_losses = 0;       % 1 --> include losses due to Johnson noise (by uSNR definition should be 0), 0 --> sigma infinite and no coil noise
include_coil_conductor_losses = 1;      % 1 --> include coil losses due to Johnson noise, 0 --> sigma infinite and no coil noise
include_ult_radiation_losses = 0;       % 1 --> (WARNING long calculation) include modes' radiation losses (average Poynting vector, no shielding)
include_coil_radiation_losses = 0;      % 1 --> (WARNING long calculation) include coils' radiation losses (average Poynting vector, no shielding)
include_circuit_losses = 0;             % 1 --> include noise (not for ultimate case) due to impedance transformation network and LNA (** currently only noise figure **)
include_psi_coil = 1;                   % 1 --> include noise correlation matrix in coil SNR optimization (always included for current patterns)
include_experimental_snr_scaling = 0;   % 1 --> include sequence related scaling factors
save_log_file = 0;                      % 1 --> save a log of keyboard input and the resulting text output

%--- virtual coils ---%
virtual_coils_flag = 1;                 % 1 --> calculate snr after coil compression
...
% coil_file = [];
coil_file = [geometrydir '32_coils_mv.mat'];
