 ...
 */
function createFileFromChunks($final_name,$temp_dir, $fileName, $chunkSize, $totalSize,$total_files) {
    ...
    // create the final destination file 
        if (($fp = fopen($final_name, 'w')) !== false) {
...
//        echo ("qui");
    ...
    $ext = pathinfo($_POST['resumableFilename'], PATHINFO_EXTENSION);

    $myfileName=uniqid() . '.' . $ext;

    _log($file);
    $w=personalSpaceData($UID,'path');


        $realfilename=$w."/".$myfileName;
        ...
        createFileFromChunks($realfilename,$temp_dir, $_POST['resumableFilename'],$_POST['resumableChunkSize'], $_POST['resumableTotalSize'],$_POST['resumableTotalChunks']);
...
//        $dbf=str_replace('/home3/cloudmrh/', '/',$realfilename);
        $dbf=$realfilename;

        $wh=personalSpaceData($UID,'http');
        $dbfh=$wh . '/' . $myfileName;
        
        $EXTID=$a->getFileIdFromExtension($ext);

        $a->addData($UID,$alias,$dbf,$t,$EXTID,filesize($realfilename),$dbfh);

        $OUT=$a->getDataInfo($UID,$dbf,$realfilename,$t);
