...
class Job
{
        ...
        public function getJOBstate($x,$uid)
    {
        ...
        if ($uid!=-1){
            ...
            $query ="SELECT * from JOBlist as j where j.status ='{$x}' order by j.dateIN DESC limit 1"; 
            $rs=QR($query);
      ...
      public function SetJobToCalc($id)
    {
     ...
     public function updateJob($x,$id,$j,$t,$FS)
    {


        $query ="update JOBlist set status='{$x}',dateOUT='{$t}',size={$FS},results='{$j}' where ID={$id}";
        $rs=QR($query);
   ...
   public function getUserJobs($id)
    {
               $query ="select * from JOBlist where UID={$id} order by dateOUT";
        $rs=QR($query);
